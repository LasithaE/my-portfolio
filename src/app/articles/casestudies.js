export const caseStudies = [
  {
    title: "Analytics & Metrics: Blinkit, Slack and Uber",
    description:
      "This is a case study where I create NSMs and OKRs (2 objectives + 3 Key Results) for Blinkit, Slack & Uber based on their quarterly business goals.",
    image: "/analytics_blinkit.png",
    tags: ["B2C", "Analytics"],
    link: "https://lasithae.substack.com/p/analytics-and-metrics-blinkit-slack",
  },
  {
    title: "Zinga: Improvement on Retention and Engagement",
    description:
      "Zinga is adding gamification with points, leaderboards & community challenges to boost engagement, retention & repeat sessions by enhancing motivation & community bonding.",
    image: "/zynga.png",
    tags: ["B2C", "Retention"],
    link: "https://lasithae.substack.com/p/zinga-improvement-on-retention-and",
  },
  {
    title:
      "Execution Questions: Analyzing Disparities in User Behavior & Root Causes of Incomplete Profiles",
    description: "A product case study on why users don’t complete profiles — digging into behavior gaps and root causes.",
    image: "/analyzing_disparities.png",
    tags: ["B2C", "Analytics"],
    link: "https://lasithae.substack.com/p/execution-questions-analyzing-disparities",
  },
  {
    title: "Tinder for Marry & Uber For Kids : Product Sense Ideation",
    description:
      "In this article I will be ideating these two topics: tinder entering the matrimonial market & uber for kids.",
    image: "/tinder_for_marry.png",
    tags: ["B2C", "Ideation"],
    link: "https://lasithae.substack.com/p/tinder-for-marry-and-uber-for-kids",
  },
  {
    title: "File Management App :1-Pager Brief + Basic Wireframes",
    description:
      "Problem Statement: A secure app for Millennials to store, manage & access personal records with privacy, quick retrieval & reminders.",
    image: "/file_management.png",
    tags: ["B2C", "Ideation"],
    link: "https://lasithae.substack.com/p/file-management-app-1-pager-brief",
  },
  {
    title: "Urban Company: RCA",
    description:
      "Problem Statement: As a PM at Urban Company, I’ll analyze why iOS ratings dropped from 4.7 to 4.1 in 6 months by reviewing user feedback, app updates, and service quality.",
    image: "/urban_company.jpg",
    tags: ["B2C", "Analytics"],
    link: "https://lasithae.substack.com/p/urban-company-rca",
  },
  {
    title: "NOBROKER: Product Discovery, Market & User Research",
    description:
      "As a PM at NoBroker in the consumer services pod, my P0 goal is to identify high-demand services & optimal pricing to drive revenue from urban residential societies in tier-1 markets.",
    image: "/no_broker.png",
    tags: ["B2C", "User Research"],
    link: "https://lasithae.substack.com/p/nobroker-product-discovery-market",
  },
  {
    title: "Duolingo: Problem Framing and Prioritisation",
    description:
      "Problem Statement: As a PM at Duolingo, the goal is to boost course completion by analyzing user behavior, identifying drop-off points, and creating strategies to maintain engagement and progress.",
    image: "/duolingo.png",
    tags: ["EdTech", "Retention"],
    link: "https://lasithae.substack.com/p/problem-framing-and-prioritisation",
  },
];
